...
try {
    ...
    Write-Host "  📖 Reading current configuration..." -NoNewline
    [xml]$config = Get-Content $configFile
    Write-Host " ✓" -ForegroundColor Green

    # Read the log4net configuration
    Write-Host "  📖 Reading log4net configuration..." -NoNewline
    ...
    if ($existingLog4netSection) {
        Write-Host "  ℹ️  log4net section already exists in config file" -ForegroundColor Yellow
    } else {
    ...
    Write-Host "`n✅ RVTools log4net configuration fixed successfully!" -ForegroundColor Green
    Write-Host "📋 Backup saved as: $backupFile" -ForegroundColor Gray
    
    Write-Host "`n🧪 Testing log4net configuration..." -ForegroundColor Cyan
    ...
    try {
        ...
        if ($log4netSection) {
            ...
            Write-Host "`n🎉 RVTools should now work without log4net errors!" -ForegroundColor Green
        } else {
    ...
    Write-Error "Failed to fix RVTools configuration: $($_.Exception.Message)"
